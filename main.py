...
def do_training(args, data_loader_test, data_loader_train, device, log_writer, model):
    ...
    print(f"Start training teacher for {args.epochs} epochs")
    ...
    print('Training time for teacher model {}'.format(total_time_str))

    args.is_train_TS = True
    args.resume = False
    do_training_student(args, data_loader_test, data_loader_train, device, log_writer, model)

def do_training_student(args, data_loader_test, data_loader_train, device, log_writer, model):
    print("actual lr: %.2e" % args.lr)
    param_groups = optim_factory.param_groups_weight_decay(model, args.weight_decay)
    optimizer = torch.optim.AdamW(param_groups, lr=args.lr, betas=(0.9, 0.95))
    print(optimizer)
    ...
    print(f"Start training student for {args.epochs_TS} epochs")
    start_time = time.time()

    for epoch in range(args.start_epoch_TS, args.epochs_TS):
        train_stats = train_one_epoch(
            model, data_loader_train,
            optimizer, device, epoch,
            log_writer=log_writer,
    ...
    total_time = time.time() - start_time
    total_time_str = str(datetime.timedelta(seconds=int(total_time)))
    print('Training time for student model {}'.format(total_time_str))
